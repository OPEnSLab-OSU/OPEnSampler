/*
  ...
  DD (int)X, sets "Draw duration" period in ms - how much time to reverse pump
  ...
  ** removed for this version : SV (int)X, sets "Sample Volume" in ml, a transform of Sample Duration, may not be 100% accurate, 2min per 250ml,
  VN (int)X, Sets the next valve/bag to place sample. Sampler sarves curent valve number during operation in EEPROM in case of power failure, it picks up where it left off
...
void listenForSerial()
{
  ...
  if (Serial.available() > 0) {
    ...
    switch (anal)
    {
      ...
      case'D':
        {
          anal = Serial.read();
          if (anal == 'D')  // then next should be D
          { // Change draw Duration
            //Serial.println(F("at FD"));
            configuration.BDDMs = Serial.parseInt();     // then an number for sample duration ms
            Serial.print(F("Draw duration set to ")); Serial.println(configuration.BDDMs);
            break;
          } // End FD anal
        }
      ...
      case'M': // M detection
        {
          directionMotor = Serial.parseInt();     // then a number for motor duration, 0=off, 1=on, -1=reverse
      ...
      case'V': // Set Valve Num
        {
          ...
          {
            configuration.VNum = Serial.parseInt() -1; // Read in and set Valve number in configuration(increments immediately, so set for -1)
