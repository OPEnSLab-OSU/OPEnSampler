...
void strobeTPICs(){
  digitalWrite(rclock,LOW); // Take rClock low to enable transfer
  delay(1);
  for(int i=0; i<4; i++) // because 4 TPICs in module
  {
    SPI.transfer(TPICBuffer[i]); //Transfer contents of TPIC buffers
    Serial.print("Transfer to TPIC:"); Serial.print(i); Serial.print(": ");Serial.println(TPICBuffer[i]); // Print to serial
  }
  delay(1);
