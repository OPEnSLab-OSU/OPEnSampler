...
void configClass::setDefaults()
{
  ...
  configData.checksum = memValidationValue;
...
*/
void configClass::getConfigData(uint8_t data[])
{
...
*/
void configClass::setDailyAlarm(unsigned int hour, unsigned int minute)
{
  setSampleHour(hour);
...
*/
void configClass::refreshPeriodicAlarm()
{
...
*/
uint8_t configClass::setSampleHour(unsigned int hour)
{
  if (hour < 24)
...
*/
uint8_t configClass::setSampleMinute(unsigned int minute)
{
  if (minute < 60)
...
*/
void configClass::setValveNumber(unsigned int valveNumber)
{
...
*/
void configClass::setPeriodicAlarm(unsigned int minutes)
{
...
*/
void configClass::setSampleDuration(unsigned long milliseconds)
{
  if (milliseconds == 0)
  {
...
/**
   ...
   (e.g. 123456789012345).
*/
void configClass::setSMSNumber(int index, char *buffer)
{
...
*/
void configClass::setFlushDuration(unsigned long milliseconds)
{
  if (milliseconds == 0)
  {
...
void configClass::setWritten(bool written)
{
  configData.written = written;
}

void configClass::setMode(Mode mode)
{
  configData.mode = mode;
}

Mode configClass::getMode()
{
  return configData.mode;
}

bool configClass::getWritten()
{
  return configData.written;
}

unsigned long configClass::getFlushDuration()
{
  return configData.flushDurationMs;
}

unsigned long configClass::getBagDrawDuration()
{
  return configData.bagDrawDurationMs;
}

unsigned long configClass::getBagFlushDuration()
{
  return configData.bagFlushDurationMs;
}

unsigned long configClass::getSampleDuration()
{
  return configData.sampleDurationMs;
}

uint8_t configClass::getSampleHour()
{
  return configData.sampleAlarmHour;
}

uint8_t configClass::getSampleMinute()
{
  return configData.sampleAlarmMinute;
}
...
/**
   ...
   (e.g. 123456789012345).
*/
char * configClass::getSMSNumber(int index)
{
  if (index >= numSMSRecipients)
    return NULL;
...
uint16_t configClass::getPeriodicAlarmLength()
{
  return configData.periodicAlarmMinutes;
}

uint8_t configClass::getValveNumber()
{
